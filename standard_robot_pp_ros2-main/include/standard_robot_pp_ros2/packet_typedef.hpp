...
const uint8_t ID_JOINT_STATE = 0x0502; // 云台pitch / yaw 角度
// Send
const uint8_t ID_ROBOT_CMD = 0x0405; // 发送运动控制指令
...
struct ReceiveJointState
{
  ...
  struct
  {
    ...
    float motor_pitch_velocity;
    float motor_yaw_angle;
    float motor_yaw_velocity;
    uint8_t shooter_id;
  } __attribute__((packed)) data;
...
struct SendRobotCmdData
{
  ...
  struct
  {
    ...
    float target_y;
    float target_turn;
  } __attribute__((packed)) robot_move_t;
